    ...
    <div className="p-6 lg:p-8 max-w-5xl">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{total} notification{total !== 1 ? "s" : ""}</p>
        </div>
        <button onClick={() => setShowCreate(true)} className="px-4 py-2.5 bg-amber-400 text-gray-950 text-sm font-semibold rounded-lg hover:bg-amber-300 self-start">+ New</button>
      </div>

      <div className="flex gap-3 mb-5">
        <select value={typeFilter} onChange={(e) => { setTypeFilter(e.target.value); setPage(1); }}
          className="px-3 py-2.5 bg-gray-900 border border-gray-800 rounded-lg text-sm text-white focus:outline-none focus:border-gray-700">
      ...
      <div className="space-y-3">
        {loading ? (
          <div className="flex justify-center py-12"><div className="animate-spin rounded-full h-6 w-6 border-b-2 border-amber-400" /></div>
        ) : notifs.length === 0 ? (
          <p className="text-center py-12 text-sm text-gray-600">No notifications found.</p>
        ) : notifs.map((n) => (
          <div key={n.id} className={`bg-gray-900 border border-gray-800 rounded-xl p-4 ${!n.is_active ? "opacity-50" : ""}`}>
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 mb-1">
                  <span className={`inline-block px-2 py-0.5 text-[11px] font-medium rounded-full border ${TYPE_COLORS[n.notification_type] || "bg-gray-800 text-gray-400 border-gray-700"}`}>{n.notification_type}</span>
                  {!n.is_active && <span className="text-[11px] text-gray-600">(inactive)</span>}
                </div>
                <h3 className="text-sm font-medium text-white">{n.title}</h3>
                <p className="text-xs text-gray-400 mt-1 line-clamp-2">{n.message}</p>
                <div className="text-[10px] text-gray-600 mt-2">by {n.created_by} · {new Date(n.created_at).toLocaleString()}</div>
              </div>
              <div className="flex gap-1 flex-shrink-0">
                <button onClick={() => handleToggle(n.id)} className="px-2 py-1 text-[11px] text-gray-400 border border-gray-700 rounded hover:bg-gray-800">
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="New Notification">
        <form onSubmit={handleCreate} className="space-y-4">
          <div>
            ...
            <input type="text" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} required className="w-full px-3 py-2.5 bg-gray-800 border border-gray-700 rounded-lg text-sm text-white focus:outline-none focus:border-amber-400/50" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-400 mb-1.5">Message</label>
            <textarea value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} required rows={4} className="w-full px-3 py-2.5 bg-gray-800 border border-gray-700 rounded-lg text-sm text-white focus:outline-none focus:border-amber-400/50 resize-none" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-gray-400 mb-1.5">Type</label>
              <select value={form.notification_type} onChange={(e) => setForm({ ...form, notification_type: e.target.value })} className="w-full px-3 py-2.5 bg-gray-800 border border-gray-700 rounded-lg text-sm text-white focus:outline-none focus:border-amber-400/50">
                <option value="announcement">Announcement</option>
                <option value="maintenance">Maintenance</option>
                <option value="alert">Alert</option>
                <option value="onboarding">Onboarding</option>
            ...
            <div>
              ...
              <select value={form.target} onChange={(e) => setForm({ ...form, target: e.target.value })} className="w-full px-3 py-2.5 bg-gray-800 border border-gray-700 rounded-lg text-sm text-white focus:outline-none focus:border-amber-400/50">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={saving} className="flex-1 py-2.5 bg-amber-400 text-gray-950 text-sm font-semibold rounded-lg hover:bg-amber-300 disabled:opacity-50">{saving ? "Creating..." : "Create"}</button>
          </div>
        </form>
      </Modal>
    </div>
